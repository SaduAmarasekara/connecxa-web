      ...
      <div className="w-full max-w-[1200px] px-4 md:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10 md:gap-y-16">
          {giftOptions.map((option, index) => (
            <div key={index} className="group flex flex-col items-center text-center md:items-start md:text-left">
              <div className="relative aspect-[16/10] w-full rounded-[20px] md:rounded-[32px] overflow-hidden bg-[#F5F5F5] mb-5 md:mb-6 shadow-[0_12px_32px_rgba(0,0,0,0.03)]">
                <Image
                  src={option.image}
                  alt={option.title}
                  fill
              ...
              <h3 className="text-[22px] md:text-[26px] font-[800] text-[#1A1A1A] mb-2 md:mb-3">
                {option.title}
              </h3>
              <p className="text-[15px] md:text-[16px] text-[#666] leading-[1.6] max-w-[500px]">
                {option.description}
              </p>
            </div>
          ))}
